...
## 2026-02-14 23:54 — Fix broken NRCan PPP link on 2 pages

Replaced broken `webapp.csrs-scrs.nrcan-rncan.gc.ca` PPP tool link (HTTP 530 — site frozen) with stable NRCan CSRS information page on both affected articles:

- `opus-ppp-failures/+page.svelte` v0.2.0 → v0.3.0 (2 occurrences)
- `space-weather-gnss-survey/+page.svelte` v0.3.0 → v0.4.0 (2 occurrences)

Old URL: `webapp.csrs-scrs.nrcan-rncan.gc.ca/geod/tools-outils/ppp.php`
New URL: `natural-resources.canada.ca/.../canadian-spatial-reference-system-csrs`
Label updated: "PPP Service" → "CSRS-PPP"

Build: 0 warnings. Tests: 46 passed. Deployed to Cloudflare Pages.

## 2026-02-14 23:50 — Dynamic route discovery for link checker

The cron worker's link checker had a hardcoded `SITE_PATHS` array that needed manual updates whenever new pages were added. Replaced with automatic route discovery:

1. **New API endpoint** (`/api/v1/routes` v0.1.0): Uses Vite `import.meta.glob` at build time to discover all `+page.svelte` files. Converts file paths to URL paths, filters out admin pages and dynamic `[param]` routes. Cached for 24h (routes only change on deploy).
2. **Cron worker updated** (`check-links.ts` v0.4.0 → v0.5.0): Now fetches `/api/v1/routes` from the live site before crawling. Falls back to the hardcoded list if the endpoint is unreachable.

**Result:** Any new page added to `src/routes/` is automatically picked up by the link checker on the next run — no code changes needed. Verified live: endpoint returns all 16 public pages.

**Files created:** `src/routes/api/v1/routes/+server.ts`
**Files changed:** `workers/cron-ingest/src/tasks/check-links.ts`

Build: 0 warnings. Tests: 46 passed. Deployed SvelteKit app + cron worker.
